...
class ModeloReportes
{
    ...
    public function dataExcelGeneral($fecha_inicio_incidencia, $fecha_fin_incidencia)
    {
        try {
            ...
            INNER JOIN CAUSA ca ON ca.id_causa = sema.id_causa
            ...
            WHERE sema.id_estado_semaforizacion IN(1,2) AND sema.id_tipo_semaforizacion IN(1,2) AND DATE_FORMAT(sema.fecha_incidencia, '%Y-%m-%d') BETWEEN '$fecha_inicio_incidencia' AND '$fecha_fin_incidencia' ";
    ...
    public function dataExcel(tipoSemaforizacion $tipoSemaforizacion, EstadoSemaforizacion $estadoSemaforizacion, $fecha_inicio_incidencia, $fecha_fin_incidencia)
    {
        try {
            ...
            INNER JOIN TIPO_SEMAFORIZACION tisema ON tisema.id_tipo_semaforizacion = sema.id_tipo_semaforizacion
            INNER JOIN usuario usu ON usu.id_usuario = sema.id_usuario  
            WHERE sema.id_estado_semaforizacion = ? AND sema.id_tipo_semaforizacion = ? AND DATE_FORMAT(sema.fecha_incidencia, '%Y-%m-%d') BETWEEN '$fecha_inicio_incidencia' AND '$fecha_fin_incidencia' ";
            $stm = $this->MYSQL->ConectarBD()->prepare($sql);
            $stm->execute(array(
